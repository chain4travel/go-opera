...
func (oracle *Oracle) resolveBlockRange(ctx context.Context, lastBlock rpc.BlockNumber, blocks int) (*types.Block, []*types.Receipt, uint64, int, error) {
	...
	if lastBlock == rpc.PendingBlockNumber {
		if pendingBlock, pendingReceipts = oracle.ethBackend.PendingBlockAndReceipts(); pendingBlock != nil {
	...
	if pendingBlock == nil {
		...
		if latestHeader, err := oracle.ethBackend.HeaderByNumber(ctx, rpc.LatestBlockNumber); err == nil {
			...
			for {
				...
				if pendingBlock != nil && blockNumber >= pendingBlock.NumberU64() {
					...
					if p, ok := oracle.historyCache.Get(cacheKey); ok {
						...
						if len(rewardPercentiles) != 0 {
							block, err := oracle.ethBackend.BlockByNumber(ctx, rpc.BlockNumber(blockNumber))
							fees.block = block.EthBlock()
							fees.err = err

							if fees.block != nil && fees.err == nil {

								fees.receipts, fees.err = oracle.ethBackend.GetReceipts(ctx, fees.block.Hash())
								fees.header = fees.block.Header()
							}
						} else {
							head, err := oracle.ethBackend.HeaderByNumber(ctx, rpc.BlockNumber(blockNumber))
							fees.header = head.EthHeader()
							fees.err = err
						}
